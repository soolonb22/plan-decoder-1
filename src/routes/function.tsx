...
export const Route = createFileRoute("/function")({ component: FunctionPage });
...
function FunctionPage() {
  ...
  const visible = WHODAS_ITEMS.filter(
    (i) => domainFilter === "all" || i.domain === domainFilter,
  );

  return (
    <MembershipGate need="core">
      <PageHeader
        title="Functional snapshot"
        lede="Inspired by WHODAS 2.0 life areas. Skip anything that does not apply. Average scores handle skipped items."
      />
      <Disclaimer>{WHODAS_DISCLAIMER}</Disclaimer>
      <Card className="mt-4">
        <p className="font-medium">Want the full practice pack?</p>
        <p className="mt-1 text-sm text-muted">
          Plan Decoder can walk through function, support needs, environment, permanency and mainstream supports, then write a practice report. Still not the NDIA.
        </p>
        <Button className="mt-3" size="sm" asChild>
          <Link to="/assessment">Open practice assessment</Link>
        </Button>
      </Card>
      ...
      <Card className="mt-6">
        ...
        <p className="mt-1 text-sm text-muted">
          Overall average {score.avgOverall.toFixed(2)} ({descriptor(score.avgOverall)}) · simple 0–100 transform {score.simple100} · {score.answered} items answered
        </p>
        {score.answered ? (
          <div className="mt-4">
        ...
        <Button
          className="mt-3"
          onClick={() => save({ date: todayISO(), items, notes })}
          disabled={score.answered === 0}
        >
          Save snapshot
        </Button>
        {history.length ? (
          <p className="mt-3 text-xs text-muted">{history.length} saved snapshot{history.length === 1 ? "" : "s"} on this device.</p>
        ) : null}
      </Card>
      {series.length > 1 ? (
        <Card className="mt-4">
