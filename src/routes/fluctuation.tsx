...
export const Route = createFileRoute("/fluctuation")({ component: FluctuationPage });
  ...
  return (
    <MembershipGate need="core">
      <PageHeader
        ...
        lede="Best-day snapshots underfund. A week of simple scores shows the real range."
      />
            ...
            {chart.map((p) => (
              <div key={p.id} className="flex flex-1 flex-col items-center gap-1">
                <div className="flex h-32 w-full items-end gap-0.5">
                  <div className="w-1/3 rounded-t bg-primary" style={{ height: `${(p.energy / max) * 100}%` }} title="Energy" />
                  <div className="w-1/3 rounded-t bg-lavender" style={{ height: `${(p.regulation / max) * 100}%` }} title="Regulation" />
                  <div className="w-1/3 rounded-t bg-leaf" style={{ height: `${(p.participation / max) * 100}%` }} title="Participation" />
                </div>
