...
export const Route = createFileRoute("/carer")({ component: CarerPage });
...
function CarerPage() {
  ...
  return (
    <MembershipGate need="core">
      <PageHeader
        title="Carer impact log"
        lede="This is not a test of love. It is a record of unpaid disability support and what it costs the household."
      />
      ...
      <div className="mt-5 space-y-3">
        {items.length === 0 ? (
          <EmptyState
            title="No carer logs yet"
            body="Overnight hours, cancelled work, and recovery days belong here."
          />
        ) : (
          items.map((item) => (
            <Card key={item.id}>
              <p className="text-xs text-muted">{formatDate(item.date)}</p>
              <p className="mt-2 text-sm whitespace-pre-wrap">{item.whatHappened}</p>
